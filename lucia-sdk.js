...
async function init(api_key, baseURL, clientId) {
  ...
  if (!sessionStorage.getItem("lid")) {
    ...
    await fetch(baseURL + "/api/sdk/init/", {
      ...
      .then((data) => {
        ...
        sessionStorage.setItem("lid", sessionId);
        window.setInterval(() => {
          const sessionId = sessionStorage.getItem("lid");
          if (sessionId) {
            const headers = {
              "Content-Type": "application/json",
              "X-API-KEY": api_key,
...
export default class Lucia {
  constructor(options) {
    ...
    init(this.api_key, this.baseURL, this.clientId);
  }
